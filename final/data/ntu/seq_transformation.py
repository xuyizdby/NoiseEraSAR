...
root_path = '/cvhci/temp/yxu/ctrgcn/ntu/'
...
def label_noise(label,noise_ratio=0,noise_type='Sym'):
    if noise_type=='Sym':
        indices = np.random.choice(np.arange(label.size), replace=False,
                           size=int(label.size * noise_ratio))
        #all classes
        all_labels=np.arange(0,60)
        #random replace label with remaining classes with equal possibility
        label[indices] = [np.random.choice(all_labels[np.where(all_labels!=i)]) for i in label[indices]]
    #todo
    elif noise_type=='Asy':
        label=label
    else:
...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path,noise_ratio):
    ...
    train_labels = label_noise(label[train_indices],noise_ratio)
...
if __name__ == '__main__':
    ...
    print(arg.noise_ratio)
    evaluations = ['CV']
    for evaluation in evaluations:
        split_dataset(skes_joints, label, performer, camera, evaluation, save_path, noise_ratio=arg.noise_ratio)
